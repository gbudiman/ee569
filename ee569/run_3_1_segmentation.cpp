...
void f_3_1_segmentation() {
  ...
  for (int i = 0; i < laws_filter.filter_banks.size(); i++) {
    ...
    vector<vector<float>> energy = s_resp.window_laws_response(15);
    filter_energy.add(energy);
    s_resp.slurp(energy);
    s_resp.write_to_file("hw3_out/P1_comb1_responses/energy_" + filter_names.at(i) + ".raw");
  }
  
  wrapped_kmeans = filter_energy.generate_kmeans(500, 425, 4, false);
  leveled = Picture("", 500, 425, COLOR_GRAY);
